...
- **`ads.txt`** at the site root, with the real publisher ID
  (`pub-6566059220538616`) already in place.
- **The AdSense script tag is live** in the `<head>` of every page
  (`ca-pub-6566059220538616`), placed right after the Google tag (gtag.js)
  block. Steps 3 and 5 below are already done — this account is now waiting
  on (or has passed) Google's review.
...
**3. ~~Apply at google.com/adsense.~~ Done.**
The verification script (`ca-pub-6566059220538616`) is already live in every
page's `<head>`.

**4. Wait for review.**
This can take anywhere from a day to a few weeks. Google checks for original
content (you have plenty — dozens of working calculators with real
explanations), a working privacy policy, and reasonable navigation, all of
which the site already satisfies.

**5. ~~Once approved, get your publisher ID.~~ Done.**
`ads.txt` and every page's AdSense script tag already use the real ID
(`pub-6566059220538616` / `ca-pub-6566059220538616`).
